...
trait IndexDAO {
  ...
  val header = List("id", "row_key", "occurrence_id", "data_hub_uid", "data_hub", "data_provider_uid", "data_provider", "data_resource_uid",
    ...
    "raw_geo_validation_status_s", "raw_occurrence_status_s", "raw_locality","raw_latitude","raw_longitude","raw_datum","raw_sex", "sensitive_locality", "monitoredList_s", "redList_s", "samplingProtocol_s") // ++ elFields ++ clFields
  ...
  def getOccIndexModel(guid: String, map: scala.collection.Map[String, String]): List[String] = {
    ...
    try {
      ...
      if (!deleted.equals("true") && map.size > 1) {
        ...
        return List(getValue("uuid", map),
          ...
          getMonitoredList(getRawScientificName(map), sciName),
	...
	def getMonitoredList(rawScientificName:String, scientificName:String) : String = {	  
